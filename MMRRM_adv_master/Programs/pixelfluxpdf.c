...
int main(int argc, char *argv[]){
	...
	if(argc==5)
		...
		/*if(OPTHIN){
			if(HIGHTS)
				sprintf(PDF_NAME,"%s/Pixelflux_PDF/PDF_TH_z%05.2f_size%04.0f_dim%04d_bin%d",MMRRM_PS_OP,zrl,box_size,dim,PDFBIN);
			else
				sprintf(PDF_NAME,"%s/Pixelflux_PDF/PDF_TN_z%05.2f_size%04.0f_dim%04d_bin%d",MMRRM_PS_OP,zrl,box_size,dim,PDFBIN);
		}
		else{
			if(HIGHTS)
				sprintf(PDF_NAME,"%s/Pixelflux_PDF/PDF_FH_z%05.2f_size%04.0f_dim%04d_bin%d",MMRRM_PS_OP,zrl,box_size,dim,PDFBIN);
			else
				sprintf(PDF_NAME,"%s/Pixelflux_PDF/PDF_FN_z%05.2f_size%04.0f_dim%04d_bin%d",MMRRM_PS_OP,zrl,box_size,dim,PDFBIN);
		}*/
