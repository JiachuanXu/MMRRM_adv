...
#	axes_PDF_RE.set_xlim(-500,500)
	axes_PDF_RE.get_yaxis().set_visible(False)

#	axes_RA_RS      = fig.add_axes([0.4,0.5,0.25,0.1])
	axes_RA_RS      = fig.add_axes([0.4,0.5,0.25,0.075])
	axes_RA_RS.set_ylim(0.89,1.12)
	axes_RA_RS.set_xlabel('log(k) [1/Mpc]')
	axes_RA_RS.set_ylabel('Ratio F/T')
	axes_RA_RS.axhline(y = 1, c = 'g', linestyle = ':')
#	axes_RA_RE      = fig.add_axes([0.65,0.5,0.25,0.1])
	axes_RA_RE      = fig.add_axes([0.65,0.5,0.25,0.075])
	axes_RA_RE.set_ylim(0.89,1.12)
	axes_RA_RE.set_xlabel('log(k) [1/Mpc]')
	axes_RA_RE.get_yaxis().set_visible(False)
	axes_RA_RE.axhline(y = 1, c = 'g', linestyle = ':')

	axes_TE_RS      = fig.add_axes([0.4,0.575,0.25,0.075])
	axes_TE_RS.set_ylabel('Ratio: H/N')
	axes_TE_RS.get_xaxis().set_visible(False)

	axes_TE_RE      = fig.add_axes([0.65,0.575,0.25,0.075])
	axes_TE_RE.get_yaxis().set_visible(False)
	axes_TE_RE.get_xaxis().set_visible(False)

	axes_PS_RS      = fig.add_axes([0.4,0.65,0.25,0.25])
...
#	axes_PS_RS.set_ylim(0.02, 1000*1.1)
	...
	axes_PS_RE      = fig.add_axes([0.65,0.65,0.25,0.25])
...
#	fp = open(file_RA_RS_wot[0],'r')
	...
	fp.close()
	axes_TE_RS.semilogx(RA_RS_karray, RA_RS_ratio, color=file_RA_RS_HN[1],linestyle=file_RA_RS_HN[2],label=file_RA_RS_HN[3])
	axes_TE_RS.legend(loc = 'upper right', fontsize = 8)

	fp = open(file_RA_RS_TF[0],'r')
	RA_RS_karray = np.array([])
	RA_RS_ratio = np.array([])
	Arg_FLAG = 1
	while Arg_FLAG:
		...
		RA_RS_ratio = np.append(RA_RS_ratio, data[1])
	fp.close()
	axes_RA_RS.semilogx(RA_RS_karray, RA_RS_ratio, color=file_RA_RS_TF[1],linestyle=file_RA_RS_TF[2],label=file_RA_RS_TF[3])
	axes_RA_RS.legend(loc = 'upper right', fontsize = 8)

	fp = open(file_RA_RE_HN[0],'r')
	...
	fp.close()
	axes_TE_RE.semilogx(RA_RE_karray, RA_RE_ratio, color=file_RA_RE_HN[1],linestyle=file_RA_RE_HN[2],label=file_RA_RE_HN[3])
	axes_TE_RE.legend(loc = 'upper right', fontsize = 8)

	fp = open(file_RA_RE_TF[0],'r')
	RA_RE_karray = np.array([])
	RA_RE_ratio = np.array([])
	Arg_FLAG = 1
	while Arg_FLAG:
		...
		RA_RE_ratio = np.append(RA_RE_ratio, data[1])
	fp.close()
	axes_RA_RE.semilogx(RA_RE_karray, RA_RE_ratio, color=file_RA_RE_TF[1],linestyle=file_RA_RE_TF[2],label=file_RA_RE_TF[3])
